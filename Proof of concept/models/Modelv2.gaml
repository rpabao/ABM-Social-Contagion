...
global{
	...
	predicate walk <- new_predicate("walk");
...
species person skills: [moving] control: /*parallel*/simple_bdi{
	...
	plan move intention:walk when: flip(prob_wander) and target=nil {
	...
	plan wearMask intention: wearMask when: not is_copying_others_no_mask{
		is_wearing_mask <- true;
		do remove_intention(wearMask, true);
	}
	
	plan removeMask intention:removeMask{
		is_wearing_mask <- false;
		do remove_intention(removeMask, true);
	}
	
	plan keep_distance intention: keep_distance when: not is_copying_others_no_distancing{
...
experiment Model type:gui{
	output{
		display map /*type: opengl*/{
			...
			overlay position: { 0, 0 } size: { 280 #px, 220 #px } background: # black transparency: 0.5 border: #black rounded: true
            {
            	float y <- 30#px;
